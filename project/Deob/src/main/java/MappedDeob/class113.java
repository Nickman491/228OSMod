...
public class class113 {
    ...
    static boolean method3177(ObjectComposition objectComposition) {
        if (objectComposition.transforms == null) {
            if (objectComposition.mapIconId == -1) return false;
            return true;
        }
        int[] nArray = objectComposition.transforms;
        int n = 0;
        while (n < nArray.length) {
            int n2 = nArray[n];
            ObjectComposition objectComposition2 = class142.getObjectDefinition(n2);
            if (objectComposition2.mapIconId != -1) {
                return true;
            }
            ++n;
        }
