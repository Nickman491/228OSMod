...
public class class133 {
    ...
    public class133(int n, Buffer buffer, boolean bl) {
        this.field1553 = buffer.readShort();
        ...
        while (true) {
            ...
            this.field1543[n2] = new class428(buffer, bl);
            this.field1546[n2][0] = buffer.method10302();
            this.field1546[n2][1] = buffer.method10302();
            this.field1546[n2][2] = buffer.method10302();
            ++n2;
        }
    }
    ...
    @Export(value="sortWorldList")
    static void sortWorldList(int n, boolean bl, int n2, boolean bl2) {
        if (World.World_worlds != null) {
            ClientPacket.doWorldSorting(0, World.World_worlds.length - 1, n, bl, n2, bl2);
        }
    }

    @ObfuscatedName(value="ae")
    @ObfuscatedSignature(descriptor="(Lcx;Lcx;IZI)I", garbageValue="-1376899254")
    @Export(value="compareWorlds")
    static int compareWorlds(World world, World world2, int n, boolean bl) {
        if (n != 1) {
            if (n == 2) {
                return world.location - world2.location;
            }
            if (n != 3) {
                if (n == 4) {
                    return world.method1977() ? (world2.method1977() ? 0 : 1) : (world2.method1977() ? -1 : 0);
                }
                if (n == 5) {
                    return world.method1965() ? (world2.method1965() ? 0 : 1) : (world2.method1965() ? -1 : 0);
                }
                if (n == 6) {
                    return world.isPvp() ? (world2.isPvp() ? 0 : 1) : (world2.isPvp() ? -1 : 0);
                }
                if (n == 7) {
                    return world.isMembersOnly() ? (world2.isMembersOnly() ? 0 : 1) : (world2.isMembersOnly() ? -1 : 0);
                }
                return world.id - world2.id;
            }
        ...
        int n2 = world.population;
        int n3 = world2.population;
        if (!bl) {
            if (n2 == -1) {
                n2 = 2001;
            }
