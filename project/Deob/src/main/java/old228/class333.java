package old228;/*
 * Decompiled with CFR 0.152.
 * 
 * Could not load the following classes:
 *  net.runelite.mapping.ObfuscatedGetter
 *  net.runelite.mapping.ObfuscatedName
 *  net.runelite.mapping.ObfuscatedSignature
 */
import java.util.ArrayList;
import java.util.LinkedList;
import java.util.List;
import net.runelite.mapping.ObfuscatedGetter;
import net.runelite.mapping.ObfuscatedName;
import net.runelite.mapping.ObfuscatedSignature;

@ObfuscatedName(value="mh")
public class class333 {
    @ObfuscatedName(value="ay")
    @ObfuscatedSignature(descriptor="Lpq;")
    static class391 field3628;
    @ObfuscatedName(value="az")
    @ObfuscatedSignature(descriptor="Lpq;")
    static class391 field3624;
    @ObfuscatedName(value="ao")
    public static ArrayList field3625;
    @ObfuscatedName(value="ad")
    public static LinkedList field3631;
    @ObfuscatedName(value="an")
    public static ArrayList field3623;
    @ObfuscatedName(value="ae")
    public static ArrayList field3626;
    @ObfuscatedName(value="ax")
    public static ArrayList field3629;
    @ObfuscatedName(value="at")
    public static final List field3627;
    @ObfuscatedName(value="ac")
    @ObfuscatedGetter(intValue=1151624493)
    static int field3630;
    @ObfuscatedName(value="au")
    @ObfuscatedGetter(intValue=2077797037)
    static int field3632;
    @ObfuscatedName(value="as")
    @ObfuscatedGetter(intValue=959714837)
    static int field3633;
    @ObfuscatedName(value="aq")
    @ObfuscatedGetter(intValue=709477055)
    static int field3634;

    static {
        field3625 = null;
        field3631 = new LinkedList();
        field3623 = new ArrayList(3);
        field3626 = new ArrayList(3);
        field3629 = new ArrayList();
        field3627 = new ArrayList();
        field3630 = 0;
        field3632 = 0;
        field3633 = 0;
        field3634 = 0;
    }
}

